...
// Exposed for profile_store.cpp to seed defaults and provide a reliable fallback.
const char* compiledDefaultProfileJson() { return PROFILE_JSON; }
const char* compiledDefaultProfileName() { return "Default"; }
...
static bool parseProfileFromJson(Profile& profile, const char* json) {
  // Keep capacity aligned with our persisted max JSON size.
...
void setup() {
  ...
  // Initialize profile storage and load active profile JSON (fallback to compiled-in default).
  const bool storeOk = profileStoreBegin();
  if (!storeOk) {
    wsLog("[profile_store] begin failed; using compiled default profile");
  } else {
    (void)profileStoreEnsureSeededDefaults();
  }

  const uint8_t activeSlot = storeOk ? profileStoreGetActiveSlot(0) : 0;
  String storedName;
  String storedJson;
  bool parsedOk = false;

  if (storeOk && profileStoreLoadSlot(activeSlot, storedName, storedJson)) {
    wsLog(String("[profile_store] loaded slot=") + (int)activeSlot + " name=\"" + storedName + "\" bytes=" + (int)storedJson.length());
    parsedOk = parseProfileFromJson(profile, storedJson.c_str());
    if (!parsedOk) {
      wsLog("[profile_store] JSON parsed but profile translation failed; falling back to default");
    }
  } else if (storeOk) {
    wsLog(String("[profile_store] no valid data for active slot=") + (int)activeSlot + "; falling back to default");
  }

  // Parse profile JSON and create PhaseProfiler AFTER phases exist.
  if (!parsedOk) {
    parsedOk = parseProfileFromJson(profile, compiledDefaultProfileJson());
  }
  ...
  wsLog("Ready. Send 'GO' via WebSocket (or Serial) to run the active stored JSON profile.");
}
